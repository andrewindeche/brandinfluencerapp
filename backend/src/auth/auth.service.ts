...
export class AuthService {
    ...
    async register(username: string, email: string, password: string): Promise<Influencer> {
      const hashedPassword = await bcrypt.hash(password, 10);
      const newInfluencer = new this.influencerModel({ username, email, password: hashedPassword });
